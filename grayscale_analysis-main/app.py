...
def load_heading():
    ...
    with st.container():
        ...
        header = st.subheader('This application performs historical portfolio analysis and future analysis with Monte Carlo Simulation')
...
def get_choices():
    ...
    years_back = st.sidebar.number_input('How Many Years Back From Today?(Maximum 5 years)', min_value=1, max_value=5, value=1)

    warning_2 = st.sidebar.write("Enter 1 Index such as SPY, 3 stock tickers, and 2 crypto tickers. Type the symbol exactly as provided by Yahoo Finance.")
    ...
    forecast_years = st.sidebar.number_input('Enter The Forecast Years For The Simulation', min_value=1, max_value=15, value=5)
    # Set the number of simulations to run_years
    sim_runs = st.sidebar.number_input('Enter The Number Of Simulations To Run (Recommended is 500 simulations, minimum 250 and maximum 1000', min_value=250, max_value=1000, value=250)
